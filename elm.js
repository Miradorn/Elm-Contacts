...
Elm.ContactBook.make = function (_elm) {
   ...
   var contactHasContent = F2(function (query,contact) {
      var contentContactFilter = function (content) {    return A2(stringHasContent,query,content.text);};
   ...
   var viewForContact = F2(function (address,contact) {
      ...
      _U.list([A2($Html.div,
              _U.list([]),
              _U.list([$Html.text(A2($Basics._op["++"],"name:",A2($Basics._op["++"],name,A2($Basics._op["++"],", company: ",company))))]))
              ,$Html.fromElement(nameField)
              ,$Html.fromElement(companyField)
              ,A2($Html.div,_U.list([]),_U.list([$Html.text("birthday:"),$Html.fromElement(birthdayField)]))
              ,A2($Html.ul,
              _U.list([]),
              _U.list([$Html.text("Addresses")
                      ,A2($Html.ul,_U.list([]),addresses)
                      ,$Html.text("Phone Numbers")
                      ,A2($Html.ul,_U.list([]),phones)
                      ,$Html.text("Emails")
                      ,A2($Html.ul,_U.list([]),mails)]))]));
   });
   ...
   var viewCategory = F3(function (category,address,model) {
      ...
      _U.list([A2($Html.h1,
              _U.list([]),
              _U.list([$Html.text(A2($Basics._op["++"],"ID: ",A2($Basics._op["++"],id,A2($Basics._op["++"]," Category Name: ",category.name.string))))]))
   ...
   var viewEmailList = F3(function (category,address,model) {
      ...
      var addButton = A2($Html.button,_U.list([A2($Html$Events.onClick,address,AddContact(category))]),_U.list([$Html.text("Add")]));
      var mappedContacts = A2(contactsWithCategory,model.contacts,category.id);
      var mailGrabber = function (contact) {    return A2($Maybe.withDefault,"",A2($Maybe.map,function (n) {    return n.text;},$List.head(contact.emails)));};
   ...
   var viewCompanies = F2(function (address,model) {
      ...
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.div,_U.list([]),_U.list([indexButton(address),A2($Html.div,_U.list([]),_U.list([A2(filterField,address,model)]))]))
              ,A2($Html.ul,_U.list([]),companiesHtml)]));
   });
   var viewTLDs = F2(function (address,model) {
      var tldMapper = function (email) {    return A2($Maybe.withDefault,"",$List.head(A2($List.drop,1,A2($String.split,"@",email.text))));};
   ...
   var viewAllContacts = F2(function (address,model) {
      ...
      var colorToContacts = $Dict.fromList(A2($List.map,
      function (cat) {
         return {ctor: "_Tuple2",_0: cat.color.string,_1: A2(contactsWithCategory,filteredContacts,cat.id)};
      },
      model.categories));
      var colorsToHTML = A2($Dict.map,
      F2(function (color,conts) {
         return A2($Html.div,_U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "color",_1: color}]))]),A2($List.map,viewForContact(address),conts));
      }),
      colorToContacts);
      var contactsHtml = $Dict.values(colorsToHTML);
      return A2($Html.div,
      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "background-color",_1: "black"}]))]),
      _U.list([A2($Html.div,_U.list([]),_U.list([indexButton(address),A2(filterField,address,model)])),A2($Html.div,_U.list([]),contactsHtml)]));
   });
   ...
   var initContactContent = F3(function (contact,id,text) {    return {id: id,contact: contact,text: text};});
   var initContact = F2(function (id,_p0) {
      var _p1 = _p0;
      var contentInitializer = initContactContent(id);
      var newAddresses = A2($List.indexedMap,contentInitializer,_p1._2);
      ...
      return {name: A2($Graphics$Input$Field.Content,_p1._0,emptySelection)
             ...
             ,phones: newPhones
             ,emails: newMails
             ,birthday: A2($Graphics$Input$Field.Content,_p1._5,emptySelection)
             ,category: _p1._6
             ,id: id};
   });
   ...
   var Contact = F8(function (a,b,c,d,e,f,g,h) {    return {name: a,company: b,addresses: c,phones: d,emails: e,birthday: f,category: g,id: h};});
   ...
   var update = F2(function (action,model) {
      ...
      {case "Insert": var newCategory = initCategory(A3(F3(function (v0,v1,v2) {    return {ctor: "_Tuple3",_0: v0,_1: v1,_2: v2};}),
         ...
         case "ShowEmailList": return {ctor: "_Tuple2",_0: _U.update(model,{viewMode: ViewEmailList(_p12._0)}),_1: $Effects.none};
         ...
         case "AddContact": var newContact = A2(initContact,
           model.nextContactID,
           {ctor: "_Tuple7",_0: "",_1: "",_2: _U.list([]),_3: _U.list([]),_4: _U.list([]),_5: "",_6: _p12._0.id});
